...
// randomSpecial generates a special character (space or punctuation) as a rune
func randomSpecial(rng *rand.Rand) rune {
	for {
		r := rune(rng.Intn(0x1000))
...
// substr extracts a substring from a unicode string
func substr(s string, start, end int) string {
	return string([]rune(s)[start:end])
}

// addSpecials adds randomly special characters (spaces, punctuations) to a string
func addSpecials(rng *rand.Rand, p string) string {
	n := rng.Intn(10)
	for i := 0; i < n; i++ {
		l := len([]rune(p))
		if l == 0 {
			p = string(randomSpecial(rng))
		} else {
			pos := rng.Intn(l)
			switch pos {
			case 0:
				p = string(randomSpecial(rng)) + p
			case l - 1:
				p = p + string(randomSpecial(rng))
			default:
				p1 := substr(p, 0, pos)
				p2 := substr(p, pos, l)
				p = p1 + string(randomSpecial(rng)) + p2
			}
		}
	}
	return p
